...
uniform float u_health_ratio = 1.;
uniform vec3 u_color_1 = vec3(0.9, 0.02, 0.1);
...
{   
    ...
    vec2 scroll_speed1 = vec2(0., 0.3);
    vec2 scroll_speed2 = vec2(0.15, 0.05);
    vec2 scroll_speed3 = vec2(0.25, 0.1);

    uv1 = v_tex_coord / vec2(0.7, 0.8);
    uv2 = v_tex_coord / vec2(0.2, 0.66);
    uv3 = v_tex_coord / vec2(0.3, 2.486);
    ...
    float alpha = 1.-smoothstep(u_health_ratio, u_health_ratio + 0.03, v_tex_coord.x);
    float alpha_flame = 1.-smoothstep(0.05, 0.5, result_r);
