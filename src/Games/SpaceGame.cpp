...
TextBubble &SpaceGame::generateWord2(bool is_correct, utils::Vector2f pos, utils::Vector2f size)
{
...
SpaceGame::SpaceGame(Renderer &window, KeyBindings &bindings, Assets &assets)
    : Game(window, bindings, assets),
      m_pos_generator(messanger, {m_font->getLineHeight() * 2, m_font->getLineHeight() * 2.f}, {800.f, 800.f})
{

    m_player = std::static_pointer_cast<PlayerShip>(m_world->insertObject([&, this](int ent_id)
                                                                          {
        PlayerShip::Spec spec;
        spec.pos = {0,0} ;
        spec.size= {100, 100};
        return std::make_shared<PlayerShip>(*m_world, spec, ent_id); }));
    ...
    m_camera.m_world_bounds = {0, 0, level_size.x, level_size.x};
    m_camera.m_can_move_x = true;
    m_camera.m_can_move_y = true;
    m_camera.setPostition(m_player->getPosition());
    m_camera.setFolowee(m_player.get());

    Meteor::Spec ms;
    ms.pos = {300, 300};
    ms.size = {100, 100};
    m_world->createObject(ms);
    ms.pos = {400, 400};
    ms.size = {50, 50};
    m_world->createObject(ms);
    Planet::Spec ps;
...
void SpaceGame::startLevel(std::string id)
{
    ...
    m_camera.m_can_move_y = true;
    m_camera.setFolowee(m_player.get());
    m_camera.threshold_rect = {0.45, 0.45, 0.1, 0.1};
}

void SpaceGame::registerSystems()
{
    auto &colllider = m_world->getCollisionSystem();
    colllider.registerResolver(ObjectType::Bullet, ObjectType::TextBubble);
    colllider.registerResolver(ObjectType::Player, ObjectType::TextBubble,
                               [](GameObject &obj1, GameObject &obj2, CollisionData c_data)
                               {
                                   auto &text = static_cast<TextBubble &>(obj2);
                                   //! do not let player through if incorrect
                                   auto surface_norm = c_data.separation_axis;
                                   auto rel_vel =obj1.m_vel - obj2.m_vel;
                                   float v_in_norm = utils::dot(surface_norm, rel_vel);
                                   if (v_in_norm > 0.f && !text.isCorrect())
                                   {
                                       obj1.m_vel -= 2*v_in_norm * surface_norm;
                                       obj1.setAngle(utils::dir2angle(obj1.m_vel));
                                       //! throw player away!
                                       if (!text.isCorrect() && text.firstTouch())
                                       {
                                           auto dr = obj1.getPosition() - obj2.getPosition();
                                           dr /= utils::norm(dr);
                                           static_cast<PlayerShip&>(obj1).health--;
                                           static_cast<PlayerShip&>(obj1).m_impulse_vel += dr * 500.f;
                                       }
                                   }
                                   text.first_touch = false;
                               });

    colllider.registerResolver(ObjectType::Player, ObjectType::Planet,
                               [](GameObject &obj1, GameObject &obj2, CollisionData c_data)
                               {
                                   ...
                                   if (dot(mvt, obj1.m_vel) < 0.f)
                                   {
                                       ...
                                       static_cast<PlayerShip &>(obj1).speed *= 0.9f;
                                       obj1.setAngle(utils::dir2angle(obj1.m_vel));
                                   }
                               });
    ...
    colllider.registerResolver(ObjectType::Meteor, ObjectType::Meteor,
                               [](GameObject &obj1, GameObject &obj2, CollisionData c_data)
                               {
...
void SpaceGame::updateImpl(float dt)
{
...
void SpaceGame::handleEventImpl(const SDL_Event &event)
{
...
void SpaceGame::drawImpl(Renderer &win)
{
